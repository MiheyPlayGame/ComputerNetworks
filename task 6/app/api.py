...
from parser_module import run_parser
...
if not DATABASE_URL:
    raise SystemExit(
        "Задайте DATABASE_URL через переменную окружения.\n"
        "Пример: DATABASE_URL=postgresql://postgres:password@yandex-music-db:5432/postgres"
    )
    ...
    if not playlist_url:
        return (
            jsonify(
                {
                    "ok": False,
                    "error": "Передайте корректную ссылку на плейлист Яндекс.Музыки в параметре url",
                }
            ),
            400,
        )
