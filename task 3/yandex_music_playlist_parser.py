...
def run_parser(use_auth=False, headless=True, playlist_url=None, save_csv=True):
    ...
    with sync_playwright() as p:
        ...
        # Помечаем прокручиваемый контейнер один раз (для scroll_one_step)
        _set_scroll_markers(page)

        # Пагинация: крутим по шагам и накапливаем треки (виртуализированный список держит в DOM только видимую часть)
        accumulated = []
        seen_keys = set()
        no_new_count = 0
        at_bottom = False
